    ...
    public void saveType() {
        ...
        if (config.contains("type.rewards")) {
            ...
            for (int i = 0; i < rewards.size(); i++) {
                ...
                if (!main.isSetting("SerializeBase64"))
                    XItemStack.serialize(new ItemBuilder(reward.getIcon()).hideAttributes().toItemStack(), Utils.getConfigurationSection(config, "type.rewards.reward_" + i + ".icon"));
                else
                    config.set("type.rewards.reward_" + i + ".icon", ItemStack64.itemStackToBase64(new ItemBuilder(reward.getIcon()).hideAttributes().toItemStack()));
