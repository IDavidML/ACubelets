...
public enum ACMaterial {
    ...
    public static final ArrayList<ACMaterial> DYES = new ArrayList<>();
    /**
     ...
     */
    public static final HashMap<ACMaterial, ACMaterial> DUPLICATED = new HashMap<ACMaterial, ACMaterial>() {{
        put(ACMaterial.MELON, ACMaterial.MELON_SLICE);
        put(ACMaterial.CARROT, ACMaterial.CARROTS);
        put(ACMaterial.POTATO, ACMaterial.POTATOES);
        put(ACMaterial.BEETROOT, ACMaterial.BEETROOTS);
        put(ACMaterial.BROWN_MUSHROOM, ACMaterial.BROWN_MUSHROOM_BLOCK);
        put(ACMaterial.BRICK, ACMaterial.BRICKS);
        put(ACMaterial.RED_MUSHROOM, ACMaterial.RED_MUSHROOM_BLOCK);
        put(ACMaterial.MAP, ACMaterial.FILLED_MAP);
        put(ACMaterial.NETHER_BRICK, ACMaterial.NETHER_BRICKS);
    }};
    /**
     * Temporary list with stained clay (terracota) and their color.
     */
    public static final HashMap<Integer, ACMaterial> CLAYS = new HashMap<Integer, ACMaterial>() {{
     ...
     */
    public static final HashMap<Integer, ACMaterial> GLASS_PANELS = new HashMap<Integer, ACMaterial>() {{
    ...
    public static final ACMaterial[] VALUES = ACMaterial.values();
    public static final String[] LEGACY_VALUES = Arrays.stream(VALUES).map(m -> m.legacy).flatMap(Arrays::stream).toArray(String[]::new);
    private static final HashMap<String, ACMaterial> CACHED_SEARCH = new HashMap<>();
    private static MinecraftVersion version;
    private static Boolean isNewVersion;

    static {
        DYES.add(ACMaterial.BLACK_DYE);
        DYES.add(ACMaterial.GREEN_DYE);
        DYES.add(ACMaterial.BROWN_DYE);
        DYES.add(ACMaterial.BLUE_DYE);
        DYES.add(ACMaterial.PURPLE_DYE);
        DYES.add(ACMaterial.CYAN_DYE);
        DYES.add(ACMaterial.LIGHT_GRAY_DYE);
        DYES.add(ACMaterial.GRAY_DYE);
        DYES.add(ACMaterial.PINK_DYE);
        DYES.add(ACMaterial.LIME_DYE);
        DYES.add(ACMaterial.YELLOW_DYE);
        DYES.add(ACMaterial.LIGHT_BLUE_DYE);
        DYES.add(ACMaterial.MAGENTA_DYE);
        DYES.add(ACMaterial.ORANGE_DYE);
        DYES.add(ACMaterial.WHITE_DYE);
    }

    private final byte data;
    private final String[] legacy;

    ACMaterial(int data, String... legacy) {
        this.data = (byte) data;
        this.legacy = legacy;
    }
     ...
     *
     * @see #matchACMaterial(String, byte)
     */
    @Nonnull
    private static ACMaterial requestOldACMaterial(@Nonnull String name, byte data) {
        ACMaterial cached = CACHED_SEARCH.get(name + "," + data);

        if (cached != null) return cached;
        Optional<ACMaterial> search = data == -1 ? Arrays.stream(ACMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name)).findFirst()
                : Arrays.stream(ACMaterial.VALUES).filter(mat -> mat.matchAnyLegacy(name) && mat.data == data).findFirst();

        if (search.isPresent()) {
            ACMaterial found = search.get();
    ...
    /**
     * Checks if ACMaterial enum contains a material with this name.
     *
     * @param name name of the material
     * @return true if ACMaterial enum has this material.
     */
    public static boolean contains(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        String formatted = format(name);
        return Arrays.stream(ACMaterial.VALUES).anyMatch(mat -> mat.name().equals(formatted));
    }
     ...
     */
    @Nullable
    public static ACMaterial matchACMaterial(@Nonnull Material material) {
        return matchACMaterial(material.name());
    }
     ...
     */
    @Nullable
    public static ACMaterial matchACMaterial(@Nonnull String name) {
        ...
        return name.contains(":") ? matchACMaterial(name.substring(0, name.indexOf(":")),
                Byte.parseByte(StringUtils.replace(name.substring(name.indexOf(":") + 1), " ", "")))
                : matchACMaterial(name, (byte) -1);
    }
    ...
    public static Material parseMaterial(@Nonnull String name, byte data) {
        return matchACMaterial(name, data).parseMaterial();
    }
    ...
    public static ACMaterial matchACMaterial(@Nonnull ItemStack item) {
        Objects.requireNonNull(item, "ItemStack cannot be null");
        return isDamageable(item.getType().name()) ? matchACMaterial(item.getType().name(), (byte) 0) :
                matchACMaterial(item.getType().name(), (byte) item.getDurability());
    }

    /**
     * Matches the argument string and its data with a ACMaterial.<br>
     *
     * @param name the name of the material
     * @param data the data byte of the material
     * @return a ACMaterial from the enum (with the same legacy name and data if in older versions.)
     */
    @Nonnull
    public static ACMaterial matchACMaterial(@Nonnull String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        name = format(name);
        ...
        if (isDuplicated(name) && !isNewVersion())
            return requestDuplicatedACMaterial(name, data);

        return requestOldACMaterial(name, data);
    }

    /**
     * Gets the ACMaterial based on the Material's ID and data.<br>
     ...
     * @return some ACMaterial, or null.
     */
    @Nullable
    public static ACMaterial matchACMaterial(int id, byte data) {
        if (id == 169) { // TODO fix that kind of issues. Temporary work around until I make a better ACMaterial class that covers IDs perfectly.
            return ACMaterial.SEA_LANTERN;
        } else if (id == 159) {
        ...
        return Arrays.stream(ACMaterial.VALUES).filter(mat -> mat.getId() == id && mat.data == data).findFirst().orElse(null);
    }
     ...
     *
     * @param name the name of the material.
     * @return the duplicated ACMaterial based on the version.
     */
    @Nullable
    private static ACMaterial requestDuplicatedACMaterial(@Nonnull String name, byte data) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        ACMaterial mat = requestOldACMaterial(name, data);
     ...
     *
     * @param name the name of the material.
     * @return the new ACMaterial of this duplicated material.
     * @see #getACMaterialIfDuplicated(String)
     */
    @Nullable
    public static ACMaterial getNewACMaterialIfDuplicated(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
     ...
     *
     * @param name the name of the material.
     * @return the other ACMaterial (key or value) of the ACMaterial (key or value) which the name was equals to.
     * @see #matchACMaterial(String, byte)
     */
    @Nullable
    public static ACMaterial getACMaterialIfDuplicated(@Nonnull String name) {
        Validate.notEmpty(name, "Material name cannot be null or empty");
        String formatted = format(name);
        Optional<Map.Entry<ACMaterial, ACMaterial>> mat = DUPLICATED.entrySet().stream().filter(m
                -> m.getKey().name().equals(formatted) || m.getValue().name().equals(formatted)).findFirst();

        if (mat.isPresent()) {
            Map.Entry<ACMaterial, ACMaterial> found = mat.get();
     ...
     */
    @Nullable
    public ACMaterial getACMaterialIfDuplicated() {
        Optional<Map.Entry<ACMaterial, ACMaterial>> mat = DUPLICATED.entrySet().stream().filter(m -> m.getKey() == this).findFirst();
    ...
    /**
     * Parses the ACMaterial as an {@link ItemStack}.
     *
    ...
    /**
     * Parses the ACMaterial as a {@link Material}.
     *
     * @return the Material related to this ACMaterial based on the server version.
     */
    @Nullable
     ...
     */
    @Nullable
    public ACMaterial suggestOldMaterialIfNew() {
        ...
        for (int i = this.legacy.length - 1; i >= 0; i--) {
            ...
            ACMaterial mat = matchACMaterial(parseLegacyVersionMaterialName(legacyName), this.data);
