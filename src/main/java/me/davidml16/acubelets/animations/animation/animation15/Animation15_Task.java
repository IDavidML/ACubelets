...
public class Animation15_Task extends Animation {
	...
	public void onTick(int time) {
		...
		if(time == 45) {
			...
			try {
				blackHole = com.cryptomorin.xseries.particles.Particles.blackhole(getMain(), 8, 4, 200, 2, 999999, ParticleDisplay.display(getCubeletBox().getLocation().clone().add(0.5, 7, 0.5), Particle.SMOKE_LARGE).withCount(1));
