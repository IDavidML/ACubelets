...
public class Animation15_Task extends Animation {
	...
	private final Set<Animation15_RotatingFlame> orbits = new HashSet<>();
	...
	public void onTick(int time) {
		...
		if (time > 0 && time < 10) Objects.requireNonNull(armorStandLocation).add(0, 0.25, 0);
		else if (time > 10 && time < 20) Objects.requireNonNull(armorStandLocation).add(0, 0.20, 0);
		else if (time > 20 && time < 30) Objects.requireNonNull(armorStandLocation).add(0, 0.15, 0);
		else if (time > 30 && time < 40) Objects.requireNonNull(armorStandLocation).add(0, 0.10, 0);
		else if (time > 40 && time < 50) Objects.requireNonNull(armorStandLocation).add(0, 0.05, 0);
		...
		if (time == 45) {
			...
			addRunnable("serpent", new Animation15_Serpent(getCubeletBox().getLocation()
					.clone()
					.add(0, 1, 0)
					.setDirection(new Vector(-1, 0, -1)), false));
			startRunnable("serpent", 0, 1);

			Location eye = armorStand.getEyeLocation().add(0.0D, 0.4D, 0.0D);
			for (Location location2 : LocationUtils.getCircle(armorStand.getLocation()
					.clone()
					.add(0, 0.75, 0), 0.25D, 50)) {
				Vector direction = location2.toVector()
						.subtract(armorStand.getLocation().clone().add(0, 0.75, 0).toVector())
						.normalize();
			...
			Animation15_RotatingFlame orbit1 = new Animation15_RotatingFlame(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, 6, 0.5), 5f, 120, true, 0);
			orbit1.runTaskTimer(getMain(), 0L, 1L);
			orbits.add(orbit1);

			Animation15_RotatingFlame orbit2 = new Animation15_RotatingFlame(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, 6, 0.5), 5f, 120, true, 40);
			orbit2.runTaskTimer(getMain(), 0L, 1L);
			orbits.add(orbit2);

			Animation15_RotatingFlame orbit3 = new Animation15_RotatingFlame(getMain(), getCubeletBox().getLocation()
					.clone()
					.add(0.5, 6, 0.5), 5f, 120, true, 80);
		...
		if (time == 150) {
			...
			UtilParticles.display(Particles.FLAME, 0.10F, 0.25F, 0.15F, armorStand.getLocation()
					.clone()
					.add(0, 1.5, 0), 5);
		...
		if (time == 185) Sounds.playSound(getCubeletBox().getLocation(), Sounds.MySound.LEVEL_UP, 0.5F, 1F);
	...
	public void onStart() {
		...
		armorStand = ASSpawner.spawn(getMain(), getCubeletBox().getLocation()
				.clone()
				.add(0.5, -0.3, 0.5), SkullUtils.itemFromBase64("eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZTBlZTc4MWI3OWIxZmRjMTRhYzA4YjUwNDQ4Y2Q5MTc0ZjcxZTE1NTUxNmUzYmY1NDJhMmQ2YzQ2NmVhY2Y4MiJ9fX0="), false, false, true);
	...
	public void onPreRewardReveal() {
		...
		getMain().getFireworkUtil()
				.spawn(getCubeletBox().getLocation()
						.clone()
						.add(0.5, 1.50, 0.5), FireworkEffect.Type.BURST, getColors().get(0), getColors().get(1));
